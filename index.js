...
const aiModel = genAI.getGenerativeModel({
  ...
  systemInstruction:
    "คุณคือ AI ผู้ช่วยของเซิร์ฟเวอร์ NaluephanBot หน้าที่ของคุณคือให้ความช่วยเหลือ ตอบคำถาม และสร้างความบันเทิงให้กับสมาชิกในเซิร์ฟเวอร์อย่างเป็นมิตรและสุภาพ",
  tools: [
    {
      googleSearch: {},
    },
...
const fortunes = [
  {
    text: "วันนี้ดวงพุ่งแรงที่สุด! มีเกณฑ์ได้รับโชคลาภก้อนโต",
    color: "#FFD700",
  },
  { text: "การงานราบรื่น มีผู้ใหญ่คอยอุปถัมภ์คำชู", color: "#00FF00" },
  { text: "ความรักสดใส คนมีคู่จะมีความสุขมาก", color: "#FF69B4" },
  {
    text: "วันนี้ดวงสร้างสรรค์กำลังมา! เหมาะแก่การเริ่มสิ่งใหม่",
    color: "#9B59B6",
  },
];
...
client.on("guildMemberAdd", async (member) => {
  ...
  const channel = member.guild.channels.cache.find(
    (ch) =>
      ...
      ch.name.includes("พูดคุย") ||
      ch.name.includes("ต้อนรับ"),
  );
  if (!channel) return;
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `สวัสดีครับ <@${member.user.id}> ยินดีต้อนรับสู่ครอบครัวของเรา\n\n📌 **สิ่งที่คุณควรทำเบื้องต้น:**\n1. เรื้อนได้เต็มที่\n2. ใครด่ามาด่ากลับอย่าไปยอม\n3. พิมพ์ \`!help\` เพื่อดูคู่มือบอท\n4. กฏคือไม่มีกฏ`,
    )
    .setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 512 }))
    .setImage(
      "https://cdn.discordapp.com/attachments/1476412853945766050/1476473830519078982/tenor.gif?ex=69a140e8&is=699fef68&hm=c291b110f00e7827ed504b7131c7892cbeecfeeebf9b261b46bcb8a9effe483f&",
    ) // ใส่รูปแบนเนอร์ต้อนรับ ต้องลงท้ายด้วย .gif / .png / .jpg เท่านั้น
    .setFooter({ text: `สมาชิกคนที่ ${member.guild.memberCount}` })
    .setTimestamp();

  channel.send({
    content: `ยินดีต้อนรับ <@${member.user.id}>! 👋`,
    embeds: [embed],
  });
});
...
client.on("messageCreate", async (message) => {
    ...
    try {
      ...
      if (error?.message?.includes("429")) {
        return message.reply(
          "⏳ ตอนนี้ AI ใช้งานหนักเกินลิมิต โปรดรอสักครู่แล้วลองใหม่ครับ",
        );
      }
  ...
  if (badWords.some((word) => message.content.includes(word))) {
    ...
    return message.channel
      .send(`⚠️ <@${message.author.id}> ระวังคำพูดด้วยครับ`)
  ...
  if (message.content === "!setup-profile") {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor("#5865F2")
        .setTitle("👤 ระบบตรวจสอบโปรไฟล์สมาชิก")
        .setDescription("กดปุ่มด้านล่างเพื่อดูสถาณะของคุณ (เห็นเฉพาะคุณ)");
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("btn_check_profile")
      ...
      message.reply("❌ บอทขาดสิทธิ์สร้างห้องครับ");
    }
  }
  ...
  if (message.content === "!เมนู" || message.content === "!menu") {
    const embed = new EmbedBuilder()
      .setColor("#5865F2")
      .setTitle("💎 KhunPor Control Center")
      .setDescription("เลือกใช้งานฟังก์ชันต่างๆ ครับ");
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("btn_luck")
        .setLabel("สุ่มดวง")
        .setStyle(ButtonStyle.Primary)
        .setEmoji("🔮"),
      new ButtonBuilder()
        .setCustomId("btn_daily")
        .setLabel("รับเงินรายวัน")
        .setStyle(ButtonStyle.Success)
        .setEmoji("💵"),
      new ButtonBuilder()
        .setCustomId("btn_ticket")
        .setLabel("แจ้งปัญหา/Ticket")
  ...
  if (
    ...
    message.content === "!วิธีใช้" ||
    message.content === "!แนะนำ"
  ) {
    const embed = new EmbedBuilder()
      .setColor("#2ECC71")
      ...
      .addFields(
        {
          name: "🤖 AI & พูดคุย",
          value:
            "`!ask <คำถาม>` - ถามคำถาม AI\n`<Tag บอท> <คำถาม>` - คุยกับ AI",
        },
        {
          name: "🛠️ ทั่วไป",
          value:
            "`!menu` - เปิดเมนูหลัก (สุ่มดวง, รับเงิน, Ticket)\n`!userinfo [@user]` - ดูข้อมูลผู้ใช้\n`!serverinfo` - ดูข้อมูลห้อง\n`!avatar [@user]` - ดูรูปโปรไฟล์\n`!ping` - เช็คความหน่วง",
        },
        {
          name: "🎮 บันเทิงและมินิเกม",
          value:
            "`!team` - เริ่มตั้งตี้สุ่มทีมเล่นเกม\n`!roll` - ทอยลูกเต๋า\n`!coin` - โยนเหรียญ\n`!rps <ค้อน/กรรไกร/กระดาษ>` - เป่ายิ้งฉุบ\n`!slots <เงินเดิมพัน>` - เล่นสล็อต",
        },
        {
          name: "💰 ระบบเศรษฐกิจ",
          value: "`!top` หรือ `!leaderboard` - ดูตารางอันดับเงิน/เลเวล",
        },
        {
          name: "⚙️ เครื่องมือ",
          value:
            "`!calc <โจทย์>` - เครื่องคิดเลข\n`!poll <คำถาม>` - สร้างโพล\n`!remindme <เวลา m/h> <ข้อความ>` - ตั้งเวลาบอกเตือน",
        },
        {
          name: "⚔️ แอดเวนเจอร์ / เกม",
          value:
            "`!hunt` - ออกล่ามอนสเตอร์รายชั่วโมง\n`!trivia` - เกมตอบคำถามความรู้ทั่วไปสุ่มหมวด",
        },
        {
          name: "👑 แอดมิน (Admin Only)",
          value:
  ...
  if (
    ...
    message.content.startsWith("!quiz")
  ) {
    const questions = [
      {
        q: "สัตว์บกที่วิ่งเร็วที่สุดในโลกคืออะไร?",
        a: ["ชีตาห์", "เสือชีตาห์", "cheetah"],
      },
      { q: "เมืองหลวงของประเทศญี่ปุ่นคืออะไร?", a: ["โตเกียว", "tokyo"] },
      {
        q: "ดาวเคราะห์ดวงใดใหญ่ที่สุดในระบบสุริยะ?",
        a: ["พฤหัส", "ดาวพฤหัสบดี", "jupiter"],
      },
      {
        q: "ผู้สร้าง Facebook คือใคร?",
        a: ["มาร์ค ซัคเคอร์เบิร์ก", "mark zuckerberg", "มาร์ค"],
      },
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `รางวัล: 50 เหรียญ / สร้างโดย: ${message.author.username}`,
      });
    ...
    // รอคำตอบ
    const filter = (m) =>
      randomQ.a.some((answer) => m.content.toLowerCase().includes(answer));
    ...
    collector.on("collect", (m) => {
      ...
      message.channel.send(
        `🎉 ยินดีด้วย! <@${m.author.id}> ตอบถูกครับ รับไป 50 เหรียญ!`,
      );
    });
    ...
    collector.on("end", (collected) => {
      if (collected.size === 0) {
        message.channel.send(
          `⏰ หมดเวลา! ไม่มีใครตอบถูกเลย คำตอบที่ถูกต้องคือ: **${randomQ.a[0]}**`,
        );
      }
    });
    return;
  ...
  if (message.content.startsWith("!hunt")) {
    ...
    if (Date.now() - lastTime < cooldown) {
      const timeLeft = Math.ceil(
        (cooldown - (Date.now() - lastTime)) / (60 * 1000),
      );
      return message.reply(
        `⏳ เหนื่อยอยู่ครับนายท่าน! รออีก ${timeLeft} นาทีค่อยออกไปล่าใหม่นะ`,
      );
    }
    ...
    db.xp.set(
      message.author.id,
      (db.xp.get(message.author.id) || 0) + enemy.xp,
    );
    db.economy.set(
      message.author.id,
      (db.economy.get(message.author.id) || 0) + enemy.coin,
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `คุณเผชิญหน้ากับ **${enemy.name}** ${enemy.emoji}\nคุณจัดการมันได้อย่างง่ายดาย!`,
      )
      .addFields(
  ...
  if (message.content.startsWith("!poll")) {
    ...
    if (!question)
      return message.reply("⚠️ กรุณาใส่คำถาม เช่น `!poll พรุ่งนี้หยุดไหม?`");
  ...
  if (message.content.startsWith("!serverinfo")) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "🏷️ ยศทั้งหมด",
          value: `${guild.roles.cache.size} ยศ`,
          inline: true,
        },
      );
    message.reply({ embeds: [embed] });
  ...
  if (
    ...
    const topMoney = [...db.economy.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    ...
    const xpText =
      topXP.length > 0
        ? topXP
            .map(
              (entry, i) =>
                `**#${i + 1}** <@${entry[0]}> - เลเวล: ${Math.floor(Math.sqrt(entry[1] / 10))} (${entry[1]} XP)`,
            )
            .join("\n")
        : "ยังไม่มีข้อมูล";
  ...
  if (message.content.startsWith("!slots")) {
    ...
    if (isNaN(bet) || bet <= 0)
      return message.reply("⚠️ ระบุจำนวนเงินเดิมพันด้วย เช่น `!slots 10`");
    ...
    if (slot1 === slot2 && slot2 === slot3) {
      ...
      db.economy.set(
        message.author.id,
        db.economy.get(message.author.id) + winAmount,
      );
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `ยอดเงินคงเหลือ: ${db.economy.get(message.author.id)} เหรียญ`,
      });
  ...
  if (message.content.startsWith("!rps")) {
    ...
    if (
      !playerChoice ||
      !["ค้อน", "กรรไกร", "กระดาษ", "rock", "paper", "scissors"].includes(
        playerChoice,
      )
    ) {
      return message.reply(
        "⚠️ ระบุตัวเลือกด้วยครับ: `!rps ค้อน` หรือ `!rps กระดาษ` หรือ `!rps กรรไกร`",
      );
    }
    ...
    if (mappedPlayer === botChoice) result = "🤝 **เสมอ!**";
      ...
      db.economy.set(
        message.author.id,
        (db.economy.get(message.author.id) || 0) + 10,
      );
  ...
  if (message.content.startsWith("!remindme")) {
    ...
    if (
      !timeStr ||
      !reminderText ||
      (!timeStr.endsWith("m") && !timeStr.endsWith("h"))
    ) {
      return message.reply(
        "⚠️ วิธีใช้: `!remindme <เวลา> <ข้อความ>` เช่น `!remindme 10m ไปตากผ้า` หรือ `!remindme 1h` (รองรับ m และ h)",
      );
    }
    ...
    message.reply(
      `✅ รับทราบ! ระบบจะแจ้งเตือนคุณในอีก **${timeLabel}** ข้างหน้าเกี่ยวกับ: *"${reminderText}"*`,
    );

    setTimeout(() => {
      message.author
        .send(
          `⏰ **ถึงเวลาแล้ว!** ระบบแจ้งเตือนของคุณ:\n> "${reminderText}"\n(ตั้งเตือนมาจากเซิร์ฟเวอร์ ${message.guild?.name || "DM"})`,
        )
        .catch(() => {
          message.channel.send(
            `⏰ <@${message.author.id}> **ถึงเวลาแล้ว!**\n> "${reminderText}"`,
          );
        });
    }, timeMs);
    return;
  ...
  if (message.content.startsWith("!giveaway")) {
    if (!message.member.permissions.has(PermissionFlagsBits.Administrator)) {
      return message.reply(
        "❌ คุณต้องเป็นแอดมินหรือมีสิทธิ์พิเศษถึงจะจัดกิจกรรมแจกของได้ครับ",
      );
    }
    ...
    if (
      ...
      return message.reply(
        "⚠️ วิธีใช้: `!giveaway <เวลา> <ของรางวัล>` เช่น `!giveaway 1m VIP Role` (รองรับ m นาที / h ชั่วโมง)",
      );
    }
    ...
    const giveMsg = await message.channel.send({
      content: "🎉 **GIVEAWAY START!** 🎉",
      embeds: [embed],
    });
    await giveMsg.react("🎉");
      ...
      try {
        ...
        if (validUsers.length === 0) {
          return message.channel.send(
            `😔 ไม่มีคนเข้าร่วมชิงรางวัล **${prize}** เลย`,
          );
        }
        ...
        message.channel.send(
          `🎉 ขอแสดงความยินดีกับ <@${winner}>! คุณโชคดีได้รับ **${prize}** 🎁 (ผู้จัด: <@${message.author.id}>)`,
        );
  ...
  if (
    message.content.startsWith("!team") ||
    message.content.startsWith("!randomteam")
  ) {
    const embed = new EmbedBuilder()
      .setColor("#3498DB")
      .setTitle("🎮 ระบบสุ่มทีม (Random Team)")
      .setDescription(
        `**โฮสต์:** <@${message.author.id}>\n\n**ผู้เข้าร่วมปัจจุบัน (0 คน):**\n- ยังไม่มีผู้เข้าร่วม`,
      )
      .setFooter({ text: "กดปุ่มด้านล่างเพื่อเข้าร่วม หรือสุ่มทีม" });

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("team_join")
        .setLabel("เข้าร่วม")
        .setStyle(ButtonStyle.Success)
        .setEmoji("✋"),
      new ButtonBuilder()
  ...
  try {
    ...
    if (interaction.customId.startsWith("role_")) {
      ...
      if (!role) {
        return interaction.reply({
          content: `❌ แอดมินยังไม่ได้สร้างยศ **${roleName}** ในเทียร์ตั้งค่าเซิร์ฟเวอร์ครับ`,
          embeds: [embed],
          ephemeral: true,
        });
      }
      ...
      if (member.roles.cache.has(role.id)) {
        ...
        return interaction.reply({
          content: `➖ ถอดยศ **${roleName}** ออกแล้ว`,
          embeds: [embed],
          ephemeral: true,
        });
      } else {
        ...
        return interaction.reply({
          content: `✅ ได้รับยศ **${roleName}** แล้ว`,
          embeds: [embed],
          ephemeral: true,
        });
      }
    ...
    if (interaction.customId === "btn_ticket") {
      try {
        ...
        await channel.send(
          `📩 **Support Ticket** สำหรับ <@${interaction.user.id}>\nกรุณาแจ้งปัญหาไว้ที่นี่ แอดมินจะมาช่วยครับ`,
        );
        await interaction.reply({
          content: `✅ สร้างห้องลับแล้ว: <#${channel.id}>`,
          ephemeral: true,
        });
      } catch (e) {
        await interaction.reply({
          content: "❌ บอทขาดสิทธิ์สร้างห้องลับ",
          embeds: [embed],
          ephemeral: true,
        });
      }
    ...
    if (interaction.customId === "btn_check_profile") {
      ...
      const embed = new EmbedBuilder()
        .setColor("#F1C40F")
        .setTitle(`👤 โปรไฟล์คุณ ${interaction.user.username}`)
        .addFields(
          { name: "💰 เงิน", value: `\`${money}\``, inline: true },
          { name: "🆙 เลเวล", value: `Level \`${level}\``, inline: true },
        );
      await interaction.reply({
        embeds: [embed],
        embeds: [embed],
    ...
    if (interaction.customId === "btn_luck") {
      ...
      if (Date.now() - lastLuck < 24 * 60 * 60 * 1000)
        return interaction.reply({
          content: "⏳ พรุ่งนี้ค่อยมาดูใหม่นะ",
          ephemeral: true,
        });
    ...
    if (interaction.customId === "btn_daily") {
      ...
      if (Date.now() - lastDaily < 24 * 60 * 60 * 1000)
        return interaction.reply({
          content: "⏳ รับเงินไปแล้วครับ",
          embeds: [embed],
          ephemeral: true,
        });
      db.economy.set(
        interaction.user.id,
        (db.economy.get(interaction.user.id) || 0) + 500,
      );
      db.lastWork.set(interaction.user.id, Date.now());
      await interaction.reply({
        content: `🎉 รับเงินรายวัน 500 เหรียญแล้ว!`,
        embeds: [embed],
        ephemeral: true,
      });
    }
    ...
    if (interaction.customId.startsWith("team_")) {
      ...
      if (!session) {
        return interaction.reply({
          content: "❌ เซสชั่นการสุ่มทีมนี้จบไปหรือถูกยกเลิกแล้วครับ",
          embeds: [embed],
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId === "team_join") {
        if (session.players.has(interaction.user.id)) {
          return interaction.reply({
            content: "⚠️ คุณอยู่ในรายชื่ออยู่แล้วครับ",
            embeds: [embed],
            ephemeral: true,
          });
        }
      ...
      if (interaction.customId === "team_leave") {
        if (!session.players.has(interaction.user.id)) {
          return interaction.reply({
            content: "⚠️ คุณยังไม่ได้เข้าร่วมครับ",
            embeds: [embed],
            ephemeral: true,
          });
        }
      ...
      if (interaction.customId === "team_start") {
        if (
          ...
          return interaction.reply({
            content:
              "❌ เฉพาะโฮสต์ที่สร้าง หรือแอดมินเท่านั้นที่สามารถกดเริ่มสุ่มได้ครับ",
            embeds: [embed],
            ephemeral: true,
          });
        }
        ...
        if (session.players.size < 2) {
          return interaction.reply({
            content: "⚠️ ต้องมีผู้เข้าร่วมอย่างน้อย 2 คนจึงจะสุ่มทีมได้ครับ",
            embeds: [embed],
            ephemeral: true,
          });
        }
        ...
        const resultEmbed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: `🔴 ทีมที่ 1 (${team1.length} คน)`,
              value:
                team1.map((id, index) => `${index + 1}. <@${id}>`).join("\n") ||
                "- ไม่มี -",
              inline: true,
            },
            {
              name: `🔵 ทีมที่ 2 (${team2.length} คน)`,
              value:
                team2.map((id, index) => `${index + 1}. <@${id}>`).join("\n") ||
                "- ไม่มี -",
              inline: true,
            },
          )
      ...
      if (interaction.customId === "team_cancel") {
        if (
          ...
          return interaction.reply({
            content:
              "❌ เฉพาะโฮสต์ที่สร้าง หรือแอดมินเท่านั้นที่สามารถยกเลิกได้ครับ",
            embeds: [embed],
            ephemeral: true,
          });
        }
...
async function updateTeamUI(interaction, session) {
  ...
  let embed;
  try {
    embed = EmbedBuilder.from(interaction.message.embeds[0]).setDescription(
      `**โฮสต์:** <@${session.hostId}>\n\n**ผู้เข้าร่วมปัจจุบัน (${playersArray.length} คน):**\n${playersList}`,
    );
  } catch (e) {
...
client.on("voiceStateUpdate", async (oldState, newState) => {
  ...
  if (
    newState.channelId === TEMP_VOICE_CREATOR_ID &&
  ...
  if (oldState.channelId && oldState.channelId !== newState.channelId) {
    ...
    if (
      oldChannel &&
      tempVoiceChannels.has(oldChannel.id) &&
